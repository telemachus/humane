...
attribute will be added by default at the end of the third section. The key for
the time attribute is `slog.TimeKey`, and the value is the time formatted with
the format string `"2006-01-02T03:04.05 MST"`. (Users can change the format of
the time attribute using `Options.TimeFormat`, and they can omit it or tweak it
further using `Options.ReplaceAttr`. See below for more information.)
...
+ `TimeFormat string`: The time format defaults to `"2006-01-02T03:04.05 MST"`.
  ...
  simple format changes.) The time Attr uses `slog.TimeKey` as its key value by
  default.
+ `AddSource bool`: This option defaults to false. If you set it to true, then
  an Attr containing `source=/path/to/source:line` will be added to each record.
  If a source Attr is present, it uses `slog.SourceKey` as its default key
  value.

A common need (e.g., for testing) is to remove the time Attr altogether. Here's
